...
class ClusterSubscriberGroup {
    ...
    constructor(subscriberGroupEmitter) {
        ...
        this.handleSubscriberConnectFailed = (error, nodeKey) => {
            ...
            const backoff = Math.min(ClusterSubscriberGroup.BASE_BACKOFF_MS * Math.pow(2, attempts), ClusterSubscriberGroup.MAX_BACKOFF_MS);
     ...
     */
    reset(clusterSlots, clusterNodes) {
            ...
            try {
                ...
                for (const [nodeKey, _] of this.subscriberToSlotsIndex) {
                    ...
                    const redis = clusterNodes.find((node) => {
                        return util_1.getNodeKey(node.options) === nodeKey;
                    });
                    if (!redis) {
