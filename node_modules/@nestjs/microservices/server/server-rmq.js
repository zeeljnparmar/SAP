...
class ServerRMQ extends server_1.Server {
    ...
    async setupChannel(channel, callback) {
        ...
        let createdQueue;
        if (this.queue === constants_1.RQM_DEFAULT_QUEUE || !noAssert) {
    ...
    initializeWildcardHandlersIfExist() {
        ...
        handlers.forEach((handler, pattern) => {
            if (pattern.includes(constants_1.RMQ_WILDCARD_ALL) ||
                pattern.includes(constants_1.RMQ_WILDCARD_SINGLE)) {
    ...
    matchRmqPattern(pattern, routingKey) {
        if (!routingKey) {
            return pattern === constants_1.RMQ_WILDCARD_ALL;
        }
        const patternSegments = pattern.split(constants_1.RMQ_SEPARATOR);
        const routingKeySegments = routingKey.split(constants_1.RMQ_SEPARATOR);
        ...
        for (const [i, currentPattern] of patternSegments.entries()) {
            ...
            if (!currentRoutingKey && currentPattern !== constants_1.RMQ_WILDCARD_ALL) {
                return false;
            }
            if (currentPattern === constants_1.RMQ_WILDCARD_ALL) {
                return i === lastIndex;
            }
            if (currentPattern !== constants_1.RMQ_WILDCARD_SINGLE &&
